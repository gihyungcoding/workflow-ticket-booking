...
/**
 * V1__create_performance.sql 의 NOT NULL/CHECK 제약을 애노테이션으로도 표현한다 (F7, AC8).
 * 테스트는 이 애노테이션에서 스키마를 생성하므로(ADR-0006) 마이그레이션과 어긋나지 않게 유지한다.
 */
@Entity
@Table(name = "performance", check = @CheckConstraint(
        name = "performance_seats_check",
        constraint = "total_seats > 0 AND available_seats >= 0 AND available_seats <= total_seats"))
