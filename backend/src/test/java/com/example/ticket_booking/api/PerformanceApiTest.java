...
class PerformanceApiTest {
    ...
    private Performance performance(String title, Instant startAt, Instant openAt, Instant closeAt,
            int totalSeats, int availableSeats, boolean cancelled) {
        return performanceRepository.save(
                new Performance(title, "테스트홀", startAt, openAt, closeAt, totalSeats, availableSeats, cancelled));
    }
    ...
    void test_sc01_목록이_기본_페이지_형식으로_반환된다() throws Exception {
        ...
        performance("공연 A", now.plus(1, ChronoUnit.DAYS), now.minus(1, ChronoUnit.DAYS), now.plus(10, ChronoUnit.DAYS), 100, 50, false);
        performance("공연 B", now.plus(2, ChronoUnit.DAYS), now.minus(1, ChronoUnit.DAYS), now.plus(10, ChronoUnit.DAYS), 100, 50, false);
        ...
        assertThat(content).extracting(row -> row.get("title")).containsExactlyInAnyOrder("공연 A", "공연 B");
    ...
    void test_sc02_statusOPEN_필터는_계산된_상태를_기준으로_적용된다() throws Exception {
        ...
        performance("공연 A", now.plus(5, ChronoUnit.DAYS), now.minus(1, ChronoUnit.DAYS), now.plus(1, ChronoUnit.DAYS), 100, 5, false);
        // Given 공연 B는 openAt <= now <= closeAt 이고 availableSeats = 0 이다 (계산상 SOLD_OUT)
        performance("공연 B", now.plus(5, ChronoUnit.DAYS), now.minus(1, ChronoUnit.DAYS), now.plus(1, ChronoUnit.DAYS), 100, 0, false);
    ...
    void test_sc03_매진된_공연은_SOLD_OUT_으로_표시된다() throws Exception {
        ...
        Performance c = performance("공연 C", now.plus(5, ChronoUnit.DAYS), now.minus(1, ChronoUnit.DAYS), now.plus(1, ChronoUnit.DAYS), 100, 0, false);

        // When GET /api/performances/{C.id} 를 호출한다
        // Then 200이 반환된다
        // Then 응답의 status 는 "SOLD_OUT" 이다
        mockMvc.perform(get("/api/performances/{id}", c.getId()))
                .andExpect(status().isOk())
    ...
    void test_sc04_취소된_공연은_CANCELLED_로_표시된다() throws Exception {
        ...
        Performance d = performance("공연 D", now.plus(5, ChronoUnit.DAYS), now.minus(1, ChronoUnit.DAYS), now.plus(1, ChronoUnit.DAYS), 100, 10, true);

        // When GET /api/performances/{D.id} 를 호출한다
        // Then 응답의 status 는 "CANCELLED" 이다
        mockMvc.perform(get("/api/performances/{id}", d.getId()))
                .andExpect(status().isOk())
    ...
    void test_sc05_지난_공연은_필터_없는_목록에서_제외된다() throws Exception {
        ...
        performance("공연 E", now.minus(1, ChronoUnit.DAYS), now.minus(10, ChronoUnit.DAYS), now.minus(2, ChronoUnit.DAYS), 100, 50, false);
        // Given 공연 F의 startAt >= now 이다
        performance("공연 F", now.plus(1, ChronoUnit.DAYS), now.minus(1, ChronoUnit.DAYS), now.plus(10, ChronoUnit.DAYS), 100, 50, false);
    ...
    void test_sc06_status필터가_일치해도_지난_공연은_여전히_제외된다() throws Exception {
        ...
        performance("공연 E", now.minus(5, ChronoUnit.DAYS), now.minus(10, ChronoUnit.DAYS), now.minus(6, ChronoUnit.DAYS), 100, 50, false);
        // Given 공연 F의 startAt >= now 이고 now > closeAt 이다 (아직 시작 전이지만 마감 시각은 지난 '조기 마감' 케이스)
        performance("공연 F", now.plus(5, ChronoUnit.DAYS), now.minus(10, ChronoUnit.DAYS), now.minus(1, ChronoUnit.DAYS), 100, 50, false);
    ...
    void test_sc07_오픈_정각에_상태가_OPEN_으로_전환된다() throws Exception {
        ...
        Performance g = performance("공연 G", closeAt.plus(1, ChronoUnit.DAYS), openAt, closeAt, 100, 1, false);
        clock.setInstant(t);

        // When GET /api/performances/{G.id} 를 호출한다
        // Then 응답의 status 는 "OPEN" 이다 (UPCOMING 이 아니다)
        mockMvc.perform(get("/api/performances/{id}", g.getId()))
                .andExpect(status().isOk())
    ...
    void test_sc08_마감_1초_전까지는_CLOSED_로_전환되지_않는다() throws Exception {
        ...
        Performance h = performance("공연 H", closeAt.plus(1, ChronoUnit.DAYS), openAt, closeAt, 100, 1, false);
        clock.setInstant(t);

        // When GET /api/performances/{H.id} 를 호출한다
        // Then 응답의 status 는 "OPEN" 이다 (CLOSED 가 아니다)
        mockMvc.perform(get("/api/performances/{id}", h.getId()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status").value("OPEN"));
    }
    ...
    void test_sc09_존재하지_않는_공연을_조회하면_404가_반환된다() throws Exception {
        ...
        mockMvc.perform(get("/api/performances/{id}", 999L))
