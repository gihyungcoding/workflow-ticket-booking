...
`frontend-design` 스킬을 로드하고 `product.md` §1~3 을 입력으로 삼아 두 파일을 만든다.

| 파일 | 무엇 |
|---|---|
| `docs/product/design.md` | 방향·근거·의미 매핑·하지 않을 것 |
| `docs/product/design-tokens.css` | 색·타이포·간격의 **정본** |

정하고 나면 **검사 가능한 것을 `constraints.yaml` 의 `DESIGN-*` 로 옮긴다.**
예시가 그 파일 주석에 있다. 여기까지 해야 "일관성 있게"가 의견이 아니라 게이트가 된다.

**한국어 UI 라면 서체에 한글 글리프가 있는지 반드시 확인한다.** Inter·Playfair Display·
Space Grotesk 처럼 흔히 추천되는 서체는 한글이 없어 시스템 폰트로 폴백되고, 그러면
정한 것이 화면에 나타나지 않는다.
...
Foundation 준비 완료
  ...
  design.md        ✓  토큰 12개, DESIGN-001~003 검증 통과   (화면이 있는 경우)
