...
취향이 아니라 하한선이다. `docs/product/design.md` §7 을 그대로 확인한다.

| 확인 | 어떻게 |
|---|---|
| `<html lang>` 이 콘텐츠 언어와 같은가 | 템플릿 기본값 `en` 이 남아 있기 쉽다 |
| `<title>` 이 템플릿 기본값이 아닌가 | `frontend`, `Vite App` 등 |
| 라이브러리가 요구하는 서체가 **실제로 로드되는가** | 네트워크 탭. 선언만 하고 설치하지 않으면 폴백된다 |
| 키보드 포커스가 눈에 보이는가 | Tab 으로 순회 |
| `prefers-reduced-motion` 존중 | 모션이 있는 경우만 |

카피는 `design.md` §5 의 의미 매핑과 대조한다 — **같은 뜻이 화면마다 같은 말인가.**
CTA 는 눌렀을 때 벌어지는 일을 그대로 말하고, 오류는 무엇을 하면 되는지 알려주며,
빈 화면은 다음 행동을 제시해야 한다.
...
| `DESIGN-*` 위반 | 아키텍처 제약과 동일하게 판정한다 (`severity` 를 따른다) |
